...
class VectorStoreManager:
    """Gestor de base de datos vectorial optimizado para memoria"""
    
    def __init__(self, config):
        self.config = config
        
        # Cargar modelo con configuración de bajo consumo
    ...
    def add_documents(self, documents: List[str], metadatas: List[Dict] = None) -> bool:
        """Añade documentos en lotes para ahorrar memoria"""
        try:
            if not documents:
            ...
            batch_size = 5  # Reducir batch size
            all_success = True
            ...
            for i in range(0, len(documents), batch_size):
                ...
                if not self._add_batch(batch_docs, batch_metadatas, i):
                    all_success = False
    ...
    def _add_batch(self, documents: List[str], metadatas: List[Dict], start_index: int) -> bool:
        """Añade un lote de documentos"""
        try:
            # Generar embeddings para el lote actual
            embeddings = self.embedding_model.encode(
                documents, 
                show_progress_bar=False,
                batch_size=2,  # Batch size pequeño
            ...
            if metadatas is None:
                metadatas = [{"source": "production", "chunk_id": start_index + j} 
                           for j in range(len(documents))]
            
            ids = [f"doc_{start_index + j}" for j in range(len(documents))]
            ...
            logger.info(f"✅ Lote de {len(documents)} documentos añadido")
            return True
    ...
    def search(self, query: str, n_results: int = 3, similarity_threshold: float = 0.3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            search_results = self.collection.query(
                ...
                n_results=min(n_results * 2, 10),  # Reducir resultados
            ...
            logger.info(f"Búsqueda: {len(filtered_results)}/{len(documents)} documentos relevantes")
